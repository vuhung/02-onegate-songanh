...
class UsersModel extends Zend_Db_Table
{
	...
	static function getDepNameByIdUser($id_u){
		$model = new UsersModel();
		$model->getDefaultAdapter()->beginTransaction();
		$query = $model->getDefaultAdapter()->query('Select dep.NAME
		from `qtht_users` u INNER JOIN `qtht_employees` e on u.`ID_EMP` = e.ID_EMP INNER JOIN qtht_departments dep on dep.ID_DEP=e.ID_DEP
		where u.`ID_U`='.$id_u);
		$re = $query->fetchAll();
		$model->getDefaultAdapter()->commit();
		//return var_dump($re);;
