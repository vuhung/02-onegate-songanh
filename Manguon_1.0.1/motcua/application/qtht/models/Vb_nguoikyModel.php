...
class Vb_nguoikyModel {
	static function getNKMC(){
		$dbAdapter = Zend_Db_Table::getDefaultAdapter();
		$usernk = Zend_Registry::get("auth")->getIdentity();
		$sql="SELECT
     				QTHT_USERS.ID_U,CONCAT(FIRSTNAME , ' ' , LASTNAME) as NAME
			  FROM
					QTHT_USERS
					INNER JOIN VB_NGUOIKY ON QTHT_USERS.`ID_U` = VB_NGUOIKY.`ID_U`
					LEFT JOIN QTHT_EMPLOYEES ON QTHT_USERS.ID_EMP=QTHT_EMPLOYEES.ID_EMP
					LEFT JOIN QTHT_DEPARTMENTS ON QTHT_DEPARTMENTS.ID_DEP = QTHT_EMPLOYEES.ID_DEP
			  WHERE
					QTHT_USERS.ISLEADER = 1
			  UNION ALL
			SELECT
     				QTHT_USERS.ID_U,CONCAT(FIRSTNAME , ' ' , LASTNAME) as NAME
			  FROM
					...
					LEFT JOIN QTHT_DEPARTMENTS ON QTHT_DEPARTMENTS.ID_DEP = QTHT_EMPLOYEES.ID_DEP
			  WHERE
					QTHT_USERS.ISLEADER = 1 AND QTHT_DEPARTMENTS.ID_DEP='".$usernk->ID_DEP."'
			";
	...
	static function ToCombo($data,$sel){
		$html="";
		foreach($data as $row){
			$html .= "<option value='".$row["NAME"]."' ".($row["NAME"]==$sel?"selected":"").">".$row["NAME"]."</option>";
		}
		return $html;
	}
