...
function Epoch(name,mode,targetelement,multiselect,year,format)
{
	this.state = 0;
	this.name = name;
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
	{
		...
		this.tgt.onblur = function () {
			if(this.calendar.format=='d/m'){
				getMinDate(this);
			}else{
				getFullDate(this);
			}
			//if(typeof(ChangeDate)=="function")eval("ChangeDate()");
...
//-----------------------------------------------------------------------------
Epoch.prototype.calConfig = function (year) //PRIVATE: initialize calendar variables
{
	//this.mode = 'flat'; //can be 'flat' or 'popup'
	this.displayYearInitial = this.curDate.getFullYear(); //the initial year to display on load
	this.displayMonthInitial = this.curDate.getMonth(); //the initial month to display on load (0-11)
	this.rangeYearLower = year-10;
	this.rangeYearUpper = year+10;
...
Epoch.prototype.setLang = function()  //all language settings for Epoch are made here.  Check Date.dateFormat() for the Date object's language settings
{
	...
	this.monthup_title = 'Tháng sau';
	this.monthdn_title = 'Tháng trước';
	this.clearbtn_caption = 'Xoá';
	this.clearbtn_title = 'Clears any dates selected on the calendar';
	this.maxrange_caption = 'Không thể chọn năm vượt quá năm làm việc';
};
//-----------------------------------------------------------------------------
Epoch.prototype.getTop = function (element) //PRIVATE: returns the absolute Top value of element, in pixels
{
    ...
    while(oNode.tagName != 'BODY' || oNode.tagName != 'body' || oNode.tagName != 'HTML') {
    	//alert(oNode.tagName);
...
CalCell.prototype.onclick = function () 
{
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		...
		else //if we can only select one cell at a time
		{
			...
			{
				owner.tgt.value = owner.selectedDates[0].dateFormat(owner.format);
				if(owner.mode == 'popup') {
					owner.hide();
				}
				//alert('sss');
				if(typeof(ChangeDate)=="function" && HasEvent==true)eval("ChangeDate()");
				if(CalendarEvent!=""){
					eval(""+CalendarEvent+"()");
				}
			}
