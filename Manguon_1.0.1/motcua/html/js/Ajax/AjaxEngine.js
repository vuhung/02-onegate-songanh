...
getvalue = function (modulename,controllername,functionname,parameter,functionjavascript){
	var aj = new AjaxEngine();
	var oXmlHttp = aj.createXMLHttp();
    oXmlHttp.open("post", "/"+modulename+"/"+controllername+"/"+functionname+"?"+parameter,true); 
 	oXmlHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        ...
        if (oXmlHttp.readyState == 4) { 
            if (oXmlHttp.status == 200) { 
            	if(oXmlHttp.responseText!=""){
					...
					if(functionjavascript!="")eval(""+functionjavascript+"()");
            		if(typeof(UpdateRet)=="function")eval("UpdateRet()");
