...
/**
	...
	static function IsFinishTransition($transition_id){
		...
		$r = $db->query($sql,$transition_id);
		$transition = $r->fetch();
		$sql = "SELECT * FROM WF_TRANSITIONS WHERE ID_P = ? and ID_A_BEGIN = ?";
		//echo "SELECT * FROM WF_TRANSITIONS WHERE ID_P = ".$transition['ID_P']." and ID_A_BEGIN = ".$transition['ID_A_END']."";
		$r = $db->query($sql,array($transition['ID_P'],$transition['ID_A_END']));
		if($r->rowCount()==0)return true;
		return false;
