<?php
	...
	class QLVBDHCommon{
		...
		static function AutoCompleteNotabchoice($data,$fieldid,$fieldname,$controlid,$controltext,$fullcompare,$style,$onokaction,$defaultid,$defaultname){
			$html="
				<script>
					var DATA_$controlid = new Array();
			";
			...
			";
			$html.="
				<input autocomplete=off onclick='cancelEvent(event)' class=autocombobox value='$defaultname' type=text style='$style' name=$controltext id=$controltext  onkeydown='at_KeyDown_alowtabnochoice(event)' onkeyup='at_Display_notab(event)' onfocus=\"at_Load('$controltext','$controlid',DATA_$controlid,".($fullcompare==true?"true":"false").",'$onokaction');\">
				<input type=hidden style='$style' name=$controlid id=$controlid value='$defaultid'>
			";
			return $html;
		}
		...
		static function GetTreeWithRoot($tree,$tablename,$fieldid,$fieldidparent,$parentvalue,$level){
			global $db;
			//echo "SELECT *,$level as LEVEL from $tablename where $fieldidparent=$parentvalue";
			$r = $db->query("SELECT *,$level as LEVEL from $tablename where $fieldidparent is null");
			$rows = $r->fetchAll();
			$r->closeCursor();
		...
		static function calendar($value, $name, $id){
			return '
				<input type="text" size=10 name="'.$name.'" id="'.$id.'" value="'.$value.'" onblur="getMinDate(this);if(typeof(ChangeDate)==\'function\')eval(\'ChangeDate()\');"><img src="/images/calendar.png" onclick="document.getElementById(\''.$id.'\').focus();var dp_cal_'.$id.' = null;
					...
					dp_cal_'.$id.'.show();HasEvent=true;CalendarEvent=\'\';"></img>
			';
		}
		...
		static function calendarFullWithEvent($value, $name, $id, $function){
			return '
				<input type="text" size=10 name="'.$name.'" id="'.$id.'" value="'.$value.'" onblur="getFullDate(this);'.$function.'();"><img src="/images/calendar.png" onclick="document.getElementById(\''.$id.'\').focus();var dp_cal_'.$id.' = null;
					dp_cal_'.$id.'  = new Epoch(\'epoch_popup\',\'popup\',document.getElementById(\''.$id.'\'),false,'.QLVBDHCommon::getYear().',\'d/m/Y\');
					dp_cal_'.$id.'.show();HasEvent=false;CalendarEvent=\'ChangeNgayHen\';"></img>
			';
		}
		static function calendarFullWithNoEvent($value, $name, $id){
			return '
				<input type="text" size=10 name="'.$name.'" id="'.$id.'" value="'.$value.'" onblur="getFullDate(this);"><img src="/images/calendar.png" onclick="document.getElementById(\''.$id.'\').focus();var dp_cal_'.$id.' = null;
					dp_cal_'.$id.'  = new Epoch(\'epoch_popup\',\'popup\',document.getElementById(\''.$id.'\'),false,'.QLVBDHCommon::getYear().',\'d/m/Y\');
					dp_cal_'.$id.'.show();HasEvent=false;CalendarEvent=\'\';"></img>
			';
		}
		/**
		...
		static function writeSelectDepartmentMultiUser($DName,$UName){
			...
			$html.="    	<div style='float:left; margin: 5px 0;'><select name=$DName id=$DName onchange='FillComboByComboExp(this,document.getElementById(\"$UName\"),$arr_user,arr_user);'>";
			$html.="        	<option value=0>--Chọn phòng--</option>";
			for($i=0;$i<count($department);$i++){
				$html.="        <option value=".$department[$i]["ID_DEP"].">".str_repeat("--",$department[$i]['LEVEL']).$department[$i]["NAME"]."</option>";
			}
			...
			$html.="<div style='float:left; margin-bottom: 5px;'><select name=$UName id=$UName multiple size=5 ondblclick=\"if(typeof(InsertIntoArr)=='function')eval('InsertIntoArr()');\"></select></div>";
			$html .= "<div class='clr'></div>";
			$html.="<script>";
			$html.="	var $arr_user = new Array();";
			for($i=0;$i<count($user);$i++)
		...
		static function writeSelectDepartmentUserWithSel($DName,$UName,$sel,$depparent){
			...
			$html.="	var $arr_user = new Array();";
			for($i=0;$i<count($user);$i++)
				$html.=$arr_user."[".$i."] = new Array('".$user[$i]['ID_DEP']."','".$user[$i]['ID_U']."','".str_replace("'","\\'", $user[$i]['NAME'])."');";
		...
		static function countdate($ngay,$arrngaynghilam){
			...
			$enddate = $curdate;
			//var_dump($enddate); var_dump($begindate); exit;
		...
		static function countdatesongayconlai($ngay,$arrngaynghilam){
			$tre = 0;
			$curdate = time();
			$istre= 1;
			if($ngay<$curdate){
		...
		static function countdate1($ngay,$arrngaynghilam,$endtime){
			$tre = 0;
			$curdate = $endtime;
			if($ngay>$curdate){
				return 0;
			...
			while(true){
				...
				if(BussinessDateModel::IsNonWorkingDate(getdate($begindate))){
					...
					$hour = date("H",$begindate);
					if($hour>=8 && $hour<12){
						//echo date("Y-m-d H:i:s",$begindate)."<br>";
						$tre++;
					}else if($hour>=13 && $hour<17){
						//echo date("Y-m-d H:i:s",$begindate)."<br>";
						$tre++;
		...
		static function getTreHan1($ngay,$hanxuly,$endtime){
			if($hanxuly==0)return 0;
			$ngay = strtotime($ngay);
			$endtime = strtotime($endtime);
			$freedate = new Zend_Session_Namespace('freedate');
			$free = $freedate->free;
			$delay = QLVBDHCommon::countdate1($ngay,$free,$endtime);
			return ($delay)-($hanxuly*8);
		}
		...
		static function countDateAll($date,$value){
			$inc = 0;
			//Đếm ngày từ ngày đến ngày hiện tại
			$value = $value;
			while(true){
		...
		static function addDate($date,$value){
			...
			while(true){
				...
				//echo date("d/m/Y H:i",$date)." $inc<br>";
				if($inc>=$value)break;
			}
			return $date;
		...
		static function InsertHSMCService($masohoso,$tentochuccanhan,$tenhoso,$trangthai,$ghichu,$dienthoai,$barcode){
            ...
            //lay dong luan chuyen cua ho so

            $dlc = $db->query("
                select CONCAT(emp.FIRSTNAME,' ',emp.LASTNAME) as NAME_U

                from ".QLVBDHCommon::Table("wf_processlogs")." wfl
                inner join ".QLVBDHCommon::Table("wf_processitems")." wfi on wfl.ID_PI = wfi.ID_PI
                inner join ".QLVBDHCommon::Table("motcua_hoso")." mc on wfi.ID_O = mc.ID_HSCV
                inner join qtht_users u on wfl.ID_U_SEND = u.ID_U
                inner join qtht_employees emp on u.ID_EMP = emp.ID_EMP
                where mc.MAHOSO = $masohoso
            ...
            $str_dlc = "";
            $i = 0;
            foreach($dlc as $itdlc){
                if($i == 1){
                    $str_dlc .= $itdlc["NAME_U"];
            ...
            //lay ten loai ho so
            $z = $db->query("SELECT TENLOAI from motcua_loai_hoso where ID_LOAIHOSO=".$u["ID_LOAIHOSO"]);
			$z = $z->fetch();
            $db->insert("SERVICES_MOTCUA_TRACUU",array("MASOHOSO"=>$masohoso,"TENTOCHUCCANHAN"=>$tentochuccanhan,"TENHOSO"=>$tenhoso,"TRANGTHAI"=>$trangthai,"GHICHU"=>$ghichu,"DIENTHOAI"=>$dienthoai,"BARCODE"=>substr(md5($masohoso),0,6),"PHONG"=>$r['NAME'],"TENLOAIHOSO"=>$z['TENLOAI'],"NHANLAI_NGAY"=>$u['NHANLAI_NGAY'],"PHUONG"=>$x['TENPHUONG'],"MA_PHONG"=>$r["CODE_DEP"],"SODOLUANCHUYEN"=>$str_dlc,"NGAY_NHANLAI"=>$u['NHANLAI_NGAY'],"DIACHI"=>$u["DIACHI"],"NGAYTIEPNHAN"=>$u["NHAN_NGAY"]));
