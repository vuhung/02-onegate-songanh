...
class Auth_UserController extends Zend_Controller_Action {
	...
	public function delayAction(){
		...
		for($i=0;$i<count($data);$i++){
			...
			if($date!="" && $hanxuly>0){
				if($type==0){
					$html .= 'document.getElementById(\'TREHAN'.$idhscv.'\').innerHTML += \''.QLVBDHCommon::trehantostrwithusername(QLVBDHCommon::getTreHan($date,$hanxuly),$date,$hanxuly,UsersModel::getEmloyeeNameByIdUser(99)).'\';';
				}else{
					$html .= 'document.getElementById(\'TREHAN'.$idhscv.'\').innerHTML += \''.QLVBDHCommon::trehantostrwithusername(QLVBDHCommon::getTreHan($date,$hanxuly),$date,$hanxuly,UsersModel::getDepNameByIdUser(99)).'\';';
				}
			}
           //$html .= 'document.getElementById(\'TREHAN'.$idhscv.'\').innerHTML +='.$ureceive;
		}
		echo $html;
		exit;
	}
	public function getnextstepAction(){
		...
		for($i=0;$i<count($data);$i++){
			...
			if(strtoupper($code)=="PRE" || strtoupper($code)=="OLD"){
				...
				if(count($action)>0){
					if($action[0]["LINK"] == "/motcua/motcua/input"  ){
						$html .= '<a href="/motcua/motcua/input/id_hscv/'.$idhscv.'/id_yeucau" >Bổ sung hồ sơ</a>';
					}else{
						...
						*/
						if($type >= 3){
							...
							if(ResourceUserModel::isAcionAlowed($user->USERNAME,$actid[0])){
								...
								$html .= '<a href="javascript:SwapIframe('.$idhscv.',\\\'/motcua/motcua/khongtrakip/id/'.$idhscv.'\\\');">Không giao trả kịp</a><br>';
							}
						}
						$actid = ResourceUserModel::getActionByUrl('hscv','choxuly','inputluuchoxuly');
						if(ResourceUserModel::isAcionAlowed($user->USERNAME,$actid[0])){
							$html .= '<a href="javascript:SwapIframe('.$idhscv.',\\\'/hscv/choxuly/inputluuchoxuly/id/'.$idhscv.'\\\');">Lưu hoãn xử lý</a><br>';
						}
					}
				}else{
					if($type >= 3){
						$html .= '<a href="javascript:SwapIframe('.$idhscv.',\\\'/motcua/motcua/trahoso/id/'.$idhscv.'/type/'.$type.'\\\');">Trả hồ sơ</a><br>';
						$html .= '<a href="javascript:SwapIframe('.$idhscv.',\\\'/hscv/nhantin/index/id/'.$idhscv.'/type/'.$type.'\\\');">Nhắn tin</a><br>';
					}else{
	...
	public function getnextstepmcAction(){
		$this->_helper->layout->disableLayout();
		global $auth;
		$user = $auth->getIdentity();
		$year = QLVBDHCommon::getYear();
		...
		for($i=0;$i<count($data);$i++){
			...
			$html = "";
			//$html = $id_tths;
			$type_next = 5; // 1: chưa xử lý, 2: đã xử lý , 3: bổ sung hồ sơ , 4: chờ bổ sung(chuyên viên), 5: không hiển thị
			$arr_tths = WFEngine::getAllTrangthaihosoByCurrentUser();
			//var_dump($arr_tths); exit;
			foreach($arr_tths as $it_tths){

				if($it_tths["ID_TTHS"] == $id_tths){
					if($it_tths["LATRANGTHAINHAN"] == 1){
						$type_next = 1;
					}
			...
			//$html = $type_next;
			if($type_next == 1){
				//trạng thái đang nhận xử lý
				$action = WFEngine::GetNextTransitions($idpi,$user->ID_U);
				if(count($action)>0){
					if($action[0]["LINK"] == "/motcua/motcua/input"  ){
						$html .= '<a href="/motcua/motcua/input/id_hscv/'.$idhscv.'/id_yeucau/'.$id_yeucau.'"'.' >Bổ sung hồ sơ</a>';
					}else{
						foreach($action as $rowa){
							$arraction = explode("/",$rowa['LINK']);
							$actid = ResourceUserModel::getActionByUrl($arraction[1],$arraction[2],$arraction[3]);
			...
			if($type_next == 3){
				//truong hop bo sung ho so
				$html .= '<a href="/motcua/motcua/input/id_hscv/'.$idhscv.'/id_yeucau/'.$id_yeucau.'"'.' >Bổ sung hồ sơ</a>';
			}

			if($type_next == 2 || $type_next >4){ //đã xử lý
				$last = WFEngine::GetCurrentTransitionInfoByIdHscv($idhscv);
				if($last['ID_U_NC']==$user->ID_U){
					if($num_step > 2){
						$html .= '<a href="javascript:SwapIframe('.$idhscv.',\\\'/hscv/hscv/thuhoi/year/'.$year.'/id/'.$idhscv.'/type/'.$type.'\\\');">Thu hồi</a><br>';
			...
			if($type_next == 4 ){//chờ bổ sung hồ sơ
			}

			if($type_next == 5 ){//vừa mới xử lý
	...
	public function adddateAction(){
		...
		$date = $param["date"];
		$value = $param["value"];
		$tocountdate =  $param["tocountdate"];
		if($tocountdate==0){
			$date = strtotime(implode("-",array_reverse(explode("/",$date)))." ".date("H:i:s"));
			if($param["all"]==1){
				echo date("d/m/Y", QLVBDHCommon::addDateAll($date,$value));
			}else{
			...
			echo QLVBDHCommon::countDateAll($value,$date);
			exit;
		}
	}
	...
	{
		$tochuc = $this->getRequest()->getParam('TENTOCHUCCANHAN');
		$custom = ResourceUserModel::getCustom($tochuc);
		$diachi = $custom['DIACHI'];
		$phuong = $custom['PHUONG'];
		if($phuong == "")
		{
			$phuong = 0;
		}
		ajax::ship("diachi",$diachi);
		ajax::ship("phuong",$phuong);
		exit;
	}
}
