...
Zapatec.Tree.Node.prototype.init = function(config) {
	...
	this.elementIcon = null; // this icon will be displayed when node is fetching its content
	...
	// clear source (to not load data one more time)
...
Zapatec.Tree.Node.prototype.create = function(){
	...
	if(
		...
		// if there is icons defined for this node - use them instead of +/-
		if(
			this.data.collapsedIcon || 
			this.data.expandedIcon ||
			this.data.fetchingIcon ||
 ...
 * @return true if node has subtree, false - in other case
 * @type boolean
 */
...
/**
 * @private
 * Actually some heavy work on creating element - getting references to its elements, putting icons etc.
...
Zapatec.Tree.Node.prototype.putIcons = function(){
	...
	if(this.expandedIcon || this.collapsedIcon || this.fetchingIcon || this.elementIcon){
		...
		if(this.expandedIcon){
			this.expandedIcon.style.display = !this.data.isExpanded || this.isFetching ? "none" : "block";
		}

		if(this.collapsedIcon){
			this.collapsedIcon.style.display = this.data.isExpanded || this.isFetching ? "none" : "block";
		}
	}
...
Zapatec.Tree.Node.prototype.onLabelDblclick = function(){
	...
	if(this.config.tree.config.editable && this.config.tree.config.editOnDblclick){
		this.config.tree.editInline.show(this.getLinkToLabelElement());
	}
...
Zapatec.Tree.Node.prototype.expand = function(){
	...
	if(this.config.quick || this.children.length == 0){
		// If quick mode is choosed or node has no children - then all nodes 
...
/**
 * Rename current node.
 * @param {Object} newLabel New node content. May container HTML.
 */
